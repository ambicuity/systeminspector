...
import type { Callback, MemData, MemLayoutData } from './types';
...
const RAM_manufacturers: Record<string, string> = {
  ...
  return new Promise<MemData>((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        ...
        let swapused = 0;
        try {
          util.powerShell('Get-CimInstance Win32_PageFileUsage | Select AllocatedBaseSize, CurrentUsage').then((stdout) => {
            const lines = stdout
              .split('\r\n')
            ...
            lines.forEach((line: any) => {
              if (line !== '') {
                ...
                swaptotal = swaptotal + (parseInt(parts[0], 10) || 0);
                swapused = swapused + (parseInt(parts[1], 10) || 0);
              }
            });
...
export function memLayout(callback?: Callback<MemLayoutData[]>): Promise<MemLayoutData[]> {
  ...
  return new Promise<MemLayoutData[]>((resolve) => {
    process.nextTick(() => {
            ...
            if (!result.length) {
              ...
              try {
                ...
                if (util.isRaspberry(lines)) {
                  ...
                  result[0].ecc = false;
                  result[0].clockSpeed = (version && version[2] && (clockSpeed as Record<string, number>)[version[2]]) || 400;
      ...
      if (_darwin) {
        exec('system_profiler SPMemoryDataType 2>/dev/null', (error, stdout) => {
          if (!error) {
