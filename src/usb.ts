...
import type { UsbData, Callback } from './types';
...
function getLinuxUsbType(type: any, name: any) {
  let result = type;
...
function parseLinuxUsb(usb: string): Partial<UsbData> {
  const result: Partial<UsbData> = {};
...
function getDarwinUsbType(name: any) {
  let result = '';
...
function parseDarwinUsb(usb: string, id?: number): Partial<UsbData> | null {
  const result: Partial<UsbData> = {};
  result.id = id;
...
function getWindowsUsbTypeCreation(creationclass: any, name: any) {
  let result = '';
...
function parseWindowsUsb(lines: any, id: any) {
  const usbType = getWindowsUsbTypeCreation(util.getValue(lines, 'CreationClassName', ':').toLowerCase(), util.getValue(lines, 'name', ':').toLowerCase());

  if (usbType) {
    const result: Partial<UsbData> = {};
    result.bus = null;
    result.deviceId = parseInt(util.getValue(lines, 'deviceid', ':'), 10) || 0;
    result.id = id;
...
export function usb(callback?: Callback<UsbData[] | null>): Promise<UsbData[] | null> {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        util.powerShell('Get-CimInstance CIM_LogicalDevice | where { $_.Description -match "USB"} | select Name,CreationClassName,DeviceId,Manufacturer | fl').then((stdout) => {
