...
const _network: Record<string, any> = {};
let _default_iface = '';
let _ifaces: Record<string, any> = {};
let _dhcpNics: any[] = [];
let _networkInterfaces: any[] = [];
let _mac: Record<string, any> = {};
let pathToIp: any;
...
function getMacAddresses() {
  ...
  if (_darwin) {
    try {
      const cmd = '/sbin/ifconfig 2>/dev/null';
...
function networkInterfaceDefault(callback: any) {
...
function getWindowsNics() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      try {
        util.powerShell(cmd).then((data: any) => {
          data = data.split('#-#-#-#');
...
function getWindowsDNSsuffixes() {
  let iface: { name?: string; dnsSuffix?: string } = {};

  const dnsSuffixes: any = {
    primaryDNS: '',
    exitCode: 0,
    ifaces: []
  };
  ...
  try {
    ...
    const ipconfigArray = ipconfig.toString().split('\r\n\r\n');

    ipconfigArray.forEach((element: any, index: any) => {
      if (index === 1) {
...
function getWindowsIfaceDNSsuffix(ifaces: any, ifacename: any) {
  let dnsSuffix = '';
...
function getWindowsWirelessIfaceSSID(interfaceName: any) {
  try {
    const result = execSync(`netsh wlan show  interface name="${interfaceName}" | findstr "SSID"`, util.execOptsWin);
    const SSID = result.toString().split('\r\n').shift();
    const parseSSID = SSID ? SSID.split(':').pop()?.trim() : '';
    return parseSSID;
  } catch {
    return 'Unknown';
  }
}
function getWindowsIEEE8021x(connectionType: any, iface: any, ifaces: any) {
  const i8021x = {
    state: 'Unknown',
    protocol: 'Unknown'
  };
  ...
  if (connectionType === 'wired' && ifaces.length > 0) {
    try {
      ...
      const iface8021xInfo = ifaces.find((element: any) => {
        return element.includes(iface + '\r\n');
      });
      const arrayIface8021xInfo = iface8021xInfo.split('\r\n');
      const state8021x = arrayIface8021xInfo.find((element: any) => {
      ...
      if (state8021x.includes('Disabled')) {
        ...
        const protocol8021x = arrayIface8021xInfo.find((element: any) => {
    ...
    try {
      ...
      if (SSID !== 'Unknown') {
        ...
        const profiles = execSync(`netsh wlan show profiles "${ifaceSanitized}"`, util.execOptsWin).toString().split('\r\n');
        i8021xState = (profiles.find((l: any) => l.indexOf('802.1X') >= 0) || '').trim();
        i8021xProtocol = (profiles.find((l: any) => l.indexOf('EAP') >= 0) || '').trim();
      }
...
function splitSectionsNics(lines: any) {
  const result: any[] = [];
  let section: any[] = [];
  lines.forEach((line: any) => {
...
function parseLinesDarwinNics(sections: any) {
  const nics: any[] = [];
...
function getDarwinNics() {
  const cmd = '/sbin/ifconfig -v 2>/dev/null';
  try {
...
function getLinuxIfaceConnectionName(interfaceName: any) {
...
function checkLinuxDCHPInterfaces(file: any) {
  let result: any[] = [];
  try {
...
function parseLinuxDHCPNics(sections: any) {
  const result: any[] = [];
  if (sections && sections.length) {
    sections.forEach((lines: any) => {
...
function getLinuxIfaceDHCPstatus(iface: any, connectionName: any, DHCPNics: any) {
  let result = false;
  if (connectionName) {
...
function getDarwinIfaceDHCPstatus(iface: any) {
  let result = false;
...
function getLinuxIfaceDNSsuffix(connectionName: any) {
  if (connectionName) {
...
function getLinuxIfaceIEEE8021xAuth(connectionName: any) {
  if (connectionName) {
...
function getLinuxIfaceIEEE8021xState(authenticationProtocol: any) {
...
function testVirtualNic(iface: any, ifaceName: any, mac: any) {
  const virtualMacs = [
    '00:00:00:00:00:00',
    '00:03:FF',
    '00:05:69',
...
function networkInterfaces(callback: any, rescan: any, defaultString: any) {
    ...
    process.nextTick(() => {
      ...
      if (_linux) {
        if (JSON.stringify(ifaces) === JSON.stringify(_ifaces) && !rescan) {
          ...
          for (let dev in ifaces) {
            ...
            if ({}.hasOwnProperty.call(ifaces, dev)) {
              ...
              ifaces[dev]?.forEach((details: any) => {
                ...
                const nodeMainVersion = parseInt(process.versions.node.split('.')[0], 10);
          ...
          _networkInterfaces = result;
          if (defaultString.toLowerCase().indexOf('default') >= 0) {
            result = result.filter((item: any) => item.default);
            if (result.length > 0) {
              result = result[0];
            } else {
              result = [];
      ...
      if (_windows) {
        if (JSON.stringify(ifaces) === JSON.stringify(_ifaces) && !rescan) {
          ...
          getWindowsNics().then((nics) => {
            (nics as any[]).forEach((nic: any) => {
              let found = false;
              Object.keys(ifaces).forEach((key) => {
                if (!found) {
                  ifaces[key]?.forEach((value: any) => {
              ...
              if (!found) {
                ifaces[nic.name] = [{ mac: nic.mac }] as any;
              }
            });
            ...
            for (let dev in ifaces) {
              ...
              if ({}.hasOwnProperty.call(ifaces, dev)) {
                ...
                dnsSuffix = getWindowsIfaceDNSsuffix((dnsSuffixes as any).ifaces, ifaceSanitized);
                let foundFirst = false;
                (nics as any[]).forEach((detail: any) => {
...
function calcNetworkSpeed(iface: any, rx_bytes: any, tx_bytes: any, operstate: any, rx_dropped: any, rx_errors: any, tx_dropped: any, tx_errors: any) {
  const result: any = {
    iface,
    operstate,
    rx_bytes,
...
function networkStats(ifaces: any, callback: any) {
  let ifacesArray: any[] = [];
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (util.isFunction(ifaces) && !callback) {
        ...
        util.restoreStringPrototype(ifaces);
        ...
        if (_windows) {
          ...
          util
            ...
            .then((stdout: any) => {
              ...
              // Network Interfaces
              networkInterfaces(false as any, undefined, undefined).then((interfaces: any) => {
                // get bytes sent, received from perfData by name
                rx_bytes = 0;
                tx_bytes = 0;
                perfData.forEach((detail: any) => {
                  (interfaces as any[]).forEach((det: any) => {
                    if (
...
function getProcessName(processes: any, pid: any) {
  let cmd = '';
...
function networkConnections(callback: any) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      const result: any[] = [];
      if (_linux || _freebsd || _openbsd || _netbsd) {
...
function networkGatewayDefault(callback: any) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      let result = '';
      if (_linux || _freebsd || _openbsd || _netbsd) {
