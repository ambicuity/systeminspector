...
const AMDBaseFrequencies: Record<string, string> = {
  8346: '1.8',
  8347: '1.9',
  8350: '2.0',
  8354: '2.2',
...
const socketTypesByName: Record<string, string> = {
  LGA1150:
...
function getSocketTypesByName(str: any) {
  let result = '';
...
function cpuManufacturer(str: any) {
  let result = str;
  str = str.toLowerCase();
...
function cpuBrandManufacturer(res: any) {
  res.brand = res.brand
    .replace(/\(R\)+/g, '®')
    .replace(/\s+/g, ' ')
    .trim();
...
function getAMDSpeed(brand: any) {
  let result = '0';
...
function getCpu() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      cpuFlags(undefined).then((flags: any) => {
        result.flags = flags;
        ...
        if (_darwin) {
          exec('sysctl machdep.cpu hw.cpufrequency_max hw.cpufrequency_min hw.packages hw.physicalcpu_max hw.ncpu hw.tbfrequency hw.cpufamily hw.cpusubfamily', (error: any, stdout: any) => {
            ...
            result.speedMin = util.getValue(lines, 'hw.cpufrequency_min') ? parseFloat(util.getValue(lines, 'hw.cpufrequency_min')) / 1000000000.0 : result.speed;
            result.speedMax = util.getValue(lines, 'hw.cpufrequency_max') ? parseFloat(util.getValue(lines, 'hw.cpufrequency_max')) / 1000000000.0 : result.speed;
            ...
            if (os.arch() === 'arm64') {
              ...
              try {
                ...
                const efficiencyCores = clusters.filter((line: any) => line.indexOf('"E"') >= 0).length;
                const performanceCores = clusters.filter((line: any) => line.indexOf('"P"') >= 0).length;
        ...
        if (_linux) {
          ...
          exec('export LC_ALL=C; lscpu; echo -n "Governor: "; cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor 2>/dev/null; echo; unset LC_ALL', (error: any, stdout: any) => {
            if (!error) {
        ...
        if (_freebsd || _openbsd || _netbsd) {
          ...
          exec('export LC_ALL=C; dmidecode -t 4; dmidecode -t 7 unset LC_ALL', (error: any, stdout: any) => {
            let cache: any[] = [];
            if (!error) {
            ...
            let sigStr = util.getValue(lines, 'signature');
            let sig = sigStr.split(',');
            ...
            for (let i = 0; i < cache.length; i++) {
              lines = cache[i].split('\n');
              let cacheTypeParts = util.getValue(lines, 'Socket Designation').toLowerCase().replace(' ', '-').split('-');
              let cacheType = cacheTypeParts.length ? cacheTypeParts[0] : '';
...
function cpuCurrentSpeed(callback: any) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      let result = getCpuCurrentSpeedSync();
      if (result.avg === 0 && _cpu_speed !== 0) {
        const currCpuSpeed = parseFloat(String(_cpu_speed));
        result = {
          min: currCpuSpeed,
          max: currCpuSpeed,
          avg: currCpuSpeed,
...
function cpuTemperature(callback: any) {
  ...
  function applyOsxTemperature(result: any) {
    ...
    if (res.main) {
      res.main = Math.round(res.main * 100) / 100;
    }
    if (res.max) {
      res.max = Math.round(res.max * 100) / 100;
    }
    if (res && res.cores && res.cores.length) {
      for (let i = 0; i < res.cores.length; i++) {
        res.cores[i] = Math.round(res.cores[i] * 100) / 100;
      }
    }
    return res;
  }

  function applyMacosTemperature(result: any) {
  ...
  function loadDarwinTemperature(result: any) {
    const arch = macArchitecture();
    const appleSilicon = arch === 'arm64' || arch === 'aarch64';
    const preferred = appleSilicon ? 'macos-temperature-sensor' : 'osx-temperature-sensor';
    const fallback = appleSilicon ? 'osx-temperature-sensor' : 'macos-temperature-sensor';

    try {
      return preferred === 'macos-temperature-sensor' ? applyMacosTemperature(result) : applyOsxTemperature(result);
    } catch {
      util.pushDiagnostic({
        feature: 'cpuTemperature',
        dependency: preferred,
        issue: 'missing_optional_package',
        message: `${preferred} is not installed or could not be loaded`,
        recommendedFix: `Install with: npm install ${preferred}`
      });
    }
    try {
      return fallback === 'macos-temperature-sensor' ? applyMacosTemperature(result) : applyOsxTemperature(result);
    } catch {
      util.pushDiagnostic({
        feature: 'cpuTemperature',
        dependency: fallback,
        issue: 'missing_optional_package',
        message: `${fallback} is not installed or could not be loaded`,
        recommendedFix: `Install with: npm install ${fallback}`
      });
    }
    return result;
  }
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        let sensorsLoadFailed = false;
        const reportMissingLinuxSensors = () => {
          if (!sensorsLoadFailed || result.main !== null || result.max !== null) {
            return;
          }
          util.pushDiagnostic({
            feature: 'cpuTemperature',
            dependency: 'lm-sensors',
            issue: 'missing_tool',
            message: 'CPU temperature could not be read from /sys, sensors, or Raspberry Pi firmware helpers',
            recommendedFix: 'Install lm-sensors, for example: sudo apt-get install lm-sensors'
          });
        };
        // CPU Chipset, Socket
        try {
        ...
        try {
          exec(cmd, (error: any, stdout: any) => {
            ...
            if (result.cores.length > 0) {
              if (result.main === null) {
                result.main = Math.round(result.cores.reduce((a: any, b: any) => a + b, 0) / result.cores.length);
              }
            ...
            exec('sensors', (error: any, stdout: any) => {
              if (error) {
                sensorsLoadFailed = true;
              } else {
                const lines = stdout.toString().split('\n');
              ...
              fs.stat('/sys/class/thermal/thermal_zone0/temp', (err: any) => {
                if (err === null) {
                  fs.readFile('/sys/class/thermal/thermal_zone0/temp', (error: any, stdout: any) => {
                    if (!error) {
                      const lines = stdout.toString().split('\n');
                      if (lines.length > 0) {
                  ...
                  exec('/opt/vc/bin/vcgencmd measure_temp', (error: any, stdout: any) => {
                    if (!error) {
                      const lines = stdout.toString().split('\n');
      ...
      if (_freebsd || _openbsd || _netbsd) {
        exec('sysctl dev.cpu | grep temp', (error: any, stdout: any) => {
          if (!error) {
            const lines = stdout.toString().split('\n');
            let sum = 0;
            lines.forEach((line: any) => {
      ...
      if (_darwin) {
        result = loadDarwinTemperature(result);
      ...
      if (_windows) {
        try {
          util.powerShell('Get-CimInstance MSAcpi_ThermalZoneTemperature -Namespace "root/wmi" | Select CurrentTemperature', { feature: 'cpuTemperature' }).then((stdout: any) => {
            const error = false;
            if (!error) {
              let sum = 0;
              const lines = stdout
              ...
              if (result.cores.length) {
                ...
                const hasOutput = stdout.toString().trim().length > 0;
                util.pushDiagnostic({
                  feature: 'cpuTemperature',
                  dependency: 'PowerShell CIM/WMI',
                  issue: hasOutput ? 'parse_error' : 'unsupported_hardware',
                  message: hasOutput ? 'Windows thermal-zone data did not contain usable temperature values' : 'Windows did not expose thermal-zone temperature values',
                  recommendedFix: 'Run with administrator privileges and confirm that the system firmware exposes ACPI thermal-zone data'
                });
              }
            }
            if (callback) {
...
function cpuFlags(callback: any) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      let result = '';
      if (_windows) {
        try {
          exec('reg query "HKEY_LOCAL_MACHINE\\HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0" /v FeatureSet', util.execOptsWin, (error: any, stdout: any) => {
            if (!error) {
      ...
      if (_linux) {
        try {
          exec('export LC_ALL=C; lscpu; unset LC_ALL', (error: any, stdout: any) => {
            if (!error) {
            ...
            if (!result) {
              fs.readFile('/proc/cpuinfo', (error: any, stdout: any) => {
                if (!error) {
                  let lines = stdout.toString().split('\n');
      ...
      if (_freebsd || _openbsd || _netbsd) {
        exec('export LC_ALL=C; dmidecode -t 4 2>/dev/null; unset LC_ALL', (error: any, stdout: any) => {
          const flags: any[] = [];
          if (!error) {
      ...
      if (_darwin) {
        exec('sysctl machdep.cpu.features', (error: any, stdout: any) => {
          if (!error) {
            let lines = stdout.toString().split('\n');
...
function cpuCache(callback: any) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_freebsd || _openbsd || _netbsd) {
        exec('export LC_ALL=C; dmidecode -t 7 2>/dev/null; unset LC_ALL', (error: any, stdout: any) => {
          let cache: any[] = [];
          if (!error) {
      ...
      if (_darwin) {
        exec('sysctl hw.l1icachesize hw.l1dcachesize hw.l2cachesize hw.l3cachesize', (error: any, stdout: any) => {
          if (!error) {
            let lines = stdout.toString().split('\n');
            lines.forEach((line: any) => {
...
function parseWinCache(linesProc: any, linesCache: any) {
  const result: any = {
    l1d: null,
    l1i: null,
    l2: null,
...
function getLoad() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      const loads = os.loadavg().map((x: any) => {
        return x / util.cores();
      });
      ...
      if (now >= 200) {
        ...
        const cpus = os.cpus().map((cpu: any) => {
          cpu.times.steal = 0;
          cpu.times.guest = 0;
          return cpu;
        });
...
function currentLoad(callback: any) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      getLoad().then((result) => {
        if (callback) {
...
function fullLoad(callback: any) {
  return new Promise((resolve) => {
    process.nextTick(() => {
