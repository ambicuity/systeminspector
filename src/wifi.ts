...
import type { Callback, WifiConnectionData, WifiInterfaceData, WifiNetworkData } from './types';
...
type NmiConnectionDetails = Partial<{
  ssid: string | null;
  uuid: string;
  type: string;
  autoconnect: boolean;
...
type WpaConnectionDetails = Partial<{
  ssid: string;
  uuid: string;
  security: string;
  freq: number;
...
function wifiDBFromQuality(quality: string | number | null): number {
  const qual = parseFloat(String(quality ?? '0'));
  if (qual < 0) {
    return 0;
  }
  if (qual >= 100) {
...
function wifiQualityFromDB(db: string | number | null): number {
  const result = 2 * (parseFloat(String(db ?? '0')) + 100);
  return result <= 100 ? result : 100;
}

const _wifi_frequencies: Record<string, number> = {
  1: 2412,
  2: 2417,
  3: 2422,
  4: 2427,
...
function ifaceListLinux(): WifiIfaceDetails[] {
  const result: WifiIfaceDetails[] = [];
...
function nmiDeviceLinux(iface: string): NmiDeviceDetails {
...
function nmiConnectionLinux(ssid: string): NmiConnectionDetails {
  const cmd = `nmcli -t connection show ${ssid} 2>/dev/null`;
  try {
    const lines = execSync(cmd, util.execOptsLinux).toString().split('\n');
...
function wpaConnectionLinux(iface: string): WpaConnectionDetails {
  if (!iface) {
    return {};
  }
...
function getWifiNetworkListNmi(): WifiNetworkData[] {
...
function parseWifiDarwin(wifiStr: string): WifiNetworkData[] {
  const result: WifiNetworkData[] = [];
  try {
...
export function wifiNetworks(callback?: Callback<WifiNetworkData[]>): Promise<WifiNetworkData[]> {
  return new Promise<WifiNetworkData[]>((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        util.powerShell(cmd).then((stdout) => {
          ...
          ssidParts.forEach((ssidPart: any) => {
            const ssidLines = ssidPart.split(os.EOL);
            if (ssidLines && ssidLines.length >= 8 && ssidLines[0].indexOf(':') >= 0) {
              const bssidsParts = ssidPart.split(' BSSID');
              bssidsParts.shift();

              bssidsParts.forEach((bssidPart: any) => {
...
export function wifiConnections(callback?: Callback<WifiConnectionData[]>): Promise<WifiConnectionData[]> {
  return new Promise<WifiConnectionData[]>((resolve) => {
    process.nextTick(() => {
...
export function wifiInterfaces(callback?: Callback<WifiInterfaceData[]>): Promise<WifiInterfaceData[]> {
  return new Promise<WifiInterfaceData[]>((resolve) => {
    process.nextTick(() => {
