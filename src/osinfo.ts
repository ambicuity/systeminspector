...
function getLogoFile(distro: any) {
  distro = distro || '';
  distro = distro.toLowerCase();
  let result: string = _platform;
  if (_windows) {
    result = 'windows';
...
function getWindowsRelease(build: any) {
...
function osInfo(callback: any) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        result.logofile = getLogoFile(undefined);
        result.release = result.kernel;
        try {
          const workload: any[] = [];
          ...
          util.promiseAll(workload).then((data: any) => {
            ...
            if (!result.codename) {
              ...
              result.codename = getWindowsRelease(buildNum) as string;
            }
            result.remoteSession = term.toString().toLowerCase().indexOf('true') >= 0;
            isUefiWindows().then((uefi) => {
              result.uefi = uefi as boolean;
              if (callback) {
                callback(result);
              }
              resolve(result);
...
function isUefiLinux() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      fs.stat('/sys/firmware/efi', (err: any) => {
        if (!err) {
          return resolve(true);
        } else {
          exec('dmesg | grep -E "EFI v"', (error: any, stdout: any) => {
            if (!error) {
              const lines = stdout.toString().split('\n');
...
function isUefiWindows() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      try {
        exec('findstr /C:"Detected boot environment" "%windir%\\Panther\\setupact.log"', util.execOptsWin, (error: any, stdout: any) => {
          if (!error) {
            const line = stdout.toString().split('\n\r')[0];
            return resolve(line.toLowerCase().indexOf('efi') >= 0);
          } else {
            exec('echo %firmware_type%', util.execOptsWin, (error: any, stdout: any) => {
              if (!error) {
...
function versions(apps: any, callback: any) {
  ...
  function checkVersionParam(apps: any) {
    if (apps === '*') {
      return {
    ...
    return { versions: {} as Record<string, any>, counter: 0 };
  }
      ...
      try {
        ...
        if (Object.hasOwn(appsObj.versions, 'postgresql')) {
          if (_linux) {
            exec('locate bin/postgres', (error: any, stdout: any) => {
              if (!error) {
                ...
                const postgresqlBin = stdout
                  .toString()
                  .split('\n')
                  .filter((p: any) => safePath.test(p.trim()))
                  .sort();
                if (postgresqlBin.length) {
                  execFile(postgresqlBin[postgresqlBin.length - 1], ['-V'], (error: any, stdout: any) => {
                    if (!error) {
                ...
                exec('psql -V', (error: any, stdout: any) => {
                  if (!error) {
                    const postgresql = stdout.toString().split('\n')[0].split(' ') || [];
                    appsObj.versions.postgresql = postgresql.length ? postgresql[postgresql.length - 1] : '';
            ...
            if (_windows) {
              ...
              exec('postgres -V', (error: any, stdout: any) => {
                if (!error) {
                  const postgresql = stdout.toString().split('\n')[0].split(' ') || [];
                  appsObj.versions.postgresql = postgresql.length ? postgresql[postgresql.length - 1] : '';
                } else {
                  exec('pg_config --version', (error: any, stdout: any) => {
                    if (!error) {
                      const postgresql = stdout.toString().split('\n')[0].split(' ') || [];
                      appsObj.versions.postgresql = postgresql.length ? postgresql[postgresql.length - 1] : '';
                    }
        ...
        if (Object.hasOwn(appsObj.versions, 'python')) {
          if (_darwin) {
            ...
            exec('python -V 2>&1', (error: any, stdout: any) => {
              if (!error) {
                const python = stdout.toString().split('\n')[0] || '';
                appsObj.versions.python = python.toLowerCase().replace('python', '').trim();
              }
        ...
        if (Object.hasOwn(appsObj.versions, 'python3')) {
          if (_darwin) {
            ...
            if (util.darwinXcodeExists() || gitHomebrewExists) {
              exec('python3 -V 2>&1', (error: any, stdout: any) => {
                if (!error) {
                  const python = stdout.toString().split('\n')[0] || '';
        ...
        if (Object.hasOwn(appsObj.versions, 'pip')) {
          if (_darwin) {
            ...
            if (util.darwinXcodeExists() || gitHomebrewExists) {
              exec('pip -V 2>&1', (error: any, stdout: any) => {
                if (!error) {
        ...
        if (Object.hasOwn(appsObj.versions, 'pip3')) {
          if (_darwin) {
            ...
            if (util.darwinXcodeExists() || gitHomebrewExists) {
              exec('pip3 -V 2>&1', (error: any, stdout: any) => {
                if (!error) {
                  const pip = stdout.toString().split('\n')[0] || '';
                  const parts = pip.split(' ');
          ...
          if (_darwin) {
            ...
            exec('/usr/libexec/java_home -V 2>&1', (error: any, stdout: any) => {
              if (!error && stdout.toString().toLowerCase().indexOf('no java runtime') === -1) {
                ...
                exec('java -version 2>&1', (error: any, stdout: any) => {
                  if (!error) {
          ...
          if ((_darwin && util.darwinXcodeExists()) || !_darwin) {
            exec('gcc -dumpversion', (error: any, stdout: any) => {
              ...
              if (appsObj.versions.gcc.indexOf('.') > -1) {
                ...
                exec('gcc --version', (error: any, stdout: any) => {
                  if (!error) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      if (_windows) {
        ...
        let result = '';
        exec('echo $SHELL', (error: any, stdout: any) => {
          if (!error) {
