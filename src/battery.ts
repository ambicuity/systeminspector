...
import type { BatteryData, Callback } from './types';
...
function parseWinBatteryPart(lines: any, designedCapacity: any, fullChargeCapacity: any) {
  ...
  if (status >= 0) {
    ...
    result.currentCapacity = Math.round((result.maxCapacity! * result.percent!) / 100);
...
function battery(callback?: Callback<BatteryData>) {
  return (
  new Promise<BatteryData>((resolve) => {
    process.nextTick(() => {
      let result: BatteryData = {
        hasBattery: false,
        cycleCount: 0,
        isCharging: false,
        designedCapacity: 0,
        ...
        try {
          ...
          util.promiseAll(workload).then((data) => {
            if (data) {
              ...
              const batteries: any[] = [];
              const hasValue = (value: any) => /\S/.test(value);
