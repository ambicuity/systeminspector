...
function getVendorFromId(id: any) {
...
function getMetalVersion(id: any) {
...
function graphics(callback: any) {
  function parseLinesDarwin(graphicsArr: any) {
    const res: any = {
      controllers: [],
      displays: []
    };
  ...
  function parseLinesLinuxControllers(lines: any) {
    ...
    try {
      ...
      pciIDs = pciIDs.filter((el: any) => el != null && el);
    } catch {
      util.noop();
    }
    let i = 1;
  ...
  function parseLinesLinuxClinfo(controllers: any, lines: any) {
    const fieldPattern = /\[([^\]]+)\]\s+(\w+)\s+(.*)/;
    const devices: any = lines.reduce((devices: any, line: any) => {
    ...
    for (const deviceId in devices) {
      ...
      if (device['CL_DEVICE_TYPE'] === 'CL_DEVICE_TYPE_GPU') {
        ...
        if (busAddress) {
          let controller: any = controllers.find((controller: any) => controller.busAddress === busAddress);
          if (!controller) {
            controller = {
              vendor: '',
              model: '',
    ...
    if (_windows) {
      try {
        ...
        if (candidates.length > 0) {
          ...
          _nvidiaSmiPath = candidates.reduce((prev: any, curr: any) => (curr && prev && curr.ctime > prev.ctime ? curr : prev))?.path || '';
        }
      } catch {
        util.noop();
      }
  ...
  function nvidiaSmi(options: any) {
  ...
  function nvidiaDevices() {
    ...
    const stdout = nvidiaSmi(undefined);
    if (!stdout) {
      return [];
    }

    const gpus = stdout.split('\n').filter(Boolean);
    let results = gpus.map((gpu: any) => {
      const splittedData = gpu.split(', ').map((value: any) => (value.includes('N/A') ? undefined : value));
  ...
  function mergeControllerNvidia(controller: any, nvidia: any) {
  ...
  function parseLinesLinuxEdid(edid: any) {
    // parsen EDID
    // --> model
    // --> resolutionx
    // --> resolutiony
  ...
  function parseLinesLinuxDisplays(lines: any, depth: any) {
    const displays: any[] = [];
        ...
        exec(cmd, (error: any, stdout: any) => {
          if (!error) {
            ...
            if (result.controllers.length === 0) {
              ...
              result.controllers = result.controllers.map((controller: any) => {
                // match by busAddress
                return mergeControllerNvidia(controller, nvidiaData.find((contr: any) => contr.pciBus.toLowerCase().endsWith(controller.busAddress.toLowerCase())) || {});
              });
            }
          }
        ...
        try {
          ...
          Promise.all(workload)
            .then((data) => {
              ...
              result.displays = result.displays.map((element: any) => {
  ...
  function parseLinesWindowsDisplaysPowershell(ssections: any, msections: any, dsections: any, tsections: any, isections: any) {
    const displays: any[] = [];
    let vendor = '';
    let model = '';
    let deviceID = '';
