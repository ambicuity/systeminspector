...
import type { UserData, Callback } from './types';
...
function parseDate(dtMon: any, dtDay: any) {
...
function parseUsersLinux(lines: any, phase: any) {
  const result: UserData[] = [];
  let result_who: UserData[] = [];
  const result_w: Partial<UserData> = {};
  let w_first = true;
  let w_header: string[] = [];
  const w_pos: number[] = [];
  let who_line: UserData[] = [];
      ...
      if (is_whopart) {
        ...
        if (w_first) {
          ...
          if (result_who.length || phase === 1) {
            ...
            who_line = [{ user: result_w.user || '', tty: result_w.tty || '', date: '', time: '', ip: '', command: '' }];
          }
...
function parseUsersDarwin(lines: any) {
  const result: UserData[] = [];
  const result_who: Array<{ user: string; tty: string; date: string; time: string }> = [];
  const result_w: Partial<UserData> = {};
  let who_line: Array<{ user: string; tty: string; date: string; time: string }> = [];
...
export function users(callback?: Callback<UserData[]>): Promise<UserData[]> {
...
function parseWinSessions(sessionParts: any) {
  const sessions: Record<string, any> = {};
  sessionParts.forEach((session: any) => {
...
function fuzzyMatch(name1: any, name2: any) {
...
function parseWinUsers(userParts: any, userQuery: any) {
  const users: any[] = [];
  userParts.forEach((user: any) => {
    ...
    if (username) {
      const quser = userQuery.filter((item: any) => fuzzyMatch(item.user, username));
      users.push({
        domain,
        user: username,
...
function parseWinLoggedOn(loggedonParts: any) {
  const loggedons: Record<string, any> = {};
  loggedonParts.forEach((loggedon: any) => {
