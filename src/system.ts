...
export function system(callback?: Callback<SystemData>): Promise<SystemData> {
  return new Promise<SystemData>((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          util.powerShell('Get-CimInstance Win32_ComputerSystemProduct | select Name,Vendor,Version,IdentifyingNumber,UUID | fl').then((stdout) => {
              ...
              util.powerShell('Get-CimInstance MS_Systeminformation -Namespace "root/wmi" | select systemsku | fl ').then((stdout) => {
                  const lines = stdout.split('\r\n');
...
function cleanDefaults(s: string): string {
...
export function bios(callback?: Callback<BiosData>): Promise<BiosData> {
  return new Promise<BiosData>((resolve) => {
    process.nextTick(() => {
      const result: BiosData = {
        vendor: '',
        version: '',
        releaseDate: '',
        revision: ''
...
export function baseboard(callback?: Callback<BaseboardData>): Promise<BaseboardData> {
  return new Promise<BaseboardData>((resolve) => {
    process.nextTick(() => {
      const result: BaseboardData = {
        manufacturer: '',
        model: '',
        version: '',
        serial: '-',
      ...
      if (_darwin) {
        ...
        workload.push(execPromise('system_profiler SPMemoryDataType 2>/dev/null'));
        util.promiseAll(workload).then((data) => {
...
function macOsChassisType(model: string): string {
...
export function chassis(callback?: Callback<ChassisData>): Promise<ChassisData> {
  const chassisTypes = [
    'Other',
    'Unknown',
    'Desktop',
  ...
  return new Promise<ChassisData>((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          util.powerShell('Get-CimInstance Win32_SystemEnclosure | select Model,Manufacturer,ChassisTypes,Version,SerialNumber,PartNumber,SKU,SMBIOSAssetTag | fl').then((stdout) => {
            const lines = stdout.toString().split('\r\n');
