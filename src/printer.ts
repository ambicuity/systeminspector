...
import type { PrinterData, Callback } from './types';
...
function parseLinuxCupsPrinter(lines: string[]): Partial<PrinterData> & { engine?: string; engineVersion?: string } {
  const result: Partial<PrinterData> & { engine?: string; engineVersion?: string } = {};
...
function parseLinuxLpstatPrinter(lines: string[], id: number): Partial<PrinterData> {
  const result: Partial<PrinterData> = {};
  result.id = id;
...
function parseDarwinPrinters(printerObject: Record<string, string>, id: number): Partial<PrinterData> {
  const result: Partial<PrinterData> = {};
...
function parseWindowsPrinters(lines: string[], id: number): Partial<PrinterData> {
  const result: Partial<PrinterData> = {};
...
export function printer(callback?: Callback<PrinterData[] | null>): Promise<PrinterData[] | null> {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        util.powerShell('Get-CimInstance Win32_Printer | select PrinterStatus,Name,DriverName,Local,Default,Shared | fl').then((stdout) => {
          {
