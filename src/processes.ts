...
const _winStatusValues: Record<number, string> = {
  0: 'unknown',
  1: 'other',
  2: 'ready',
  3: 'running',
...
function parseElapsedTime(etime: any) {
...
function services(srv: any, callback: any) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (srv) {
        let srvString = '';
        util.restoreStringPrototype(srvString);
...
function calcProcStatWin(procStat: any, all: any, _cpu_old: any) {
  // calc
  let cpuu = 0;
  let cpus = 0;
...
function processes(callback: any) {
  let parsedhead: any[] = [];

  function getName(command: any) {
    command = command || '';
  ...
  function parseProcesses2(lines: any) {
    ...
    function parseElapsed(etime: any) {
      let started = '';
...
function processLoad(proc: any, callback: any) {
  // fallback - if only callback is given
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      let processesString = '';
      util.restoreStringPrototype(processesString);
      ...
      if (procSanitized && processes.length && processes[0] !== '------') {
        if (_windows) {
          try {
            util.powerShell('Get-CimInstance Win32_Process | select ProcessId,Caption,UserModeTime,KernelModeTime,WorkingSetSize | ConvertTo-Json -compress').then((stdout: any) => {
              const error = false;
              if (!error) {
                const procStats: any[] = [];
                const list_new: Record<string, any> = {};
        ...
        if (_darwin || _linux || _freebsd || _openbsd || _netbsd) {
          ...
          util.execSafe('ps', params).then((stdout: any) => {
            if (stdout) {
              const procStats: any[] = [];
              const lines = stdout
                .toString()
