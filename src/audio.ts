...
import type { AudioData, Callback } from './types';
...
function parseAudioType(str: any, input: any, output: any) {
  str = str.toLowerCase();
  let result = '';
...
function getLinuxAudioPci() {
  ...
  const result: any[] = [];
  try {
    const parts = execSync(cmd, util.execOptsLinux as import('child_process').ExecSyncOptions).toString().split('\n\n');
    parts.forEach((element) => {
      const lines = element.split('\n');
      if (lines && lines.length && lines[0].toLowerCase().indexOf('audio') >= 0) {
        const audio: { slotId?: string; driver?: string } = {};
...
function parseWinAudioStatus(n: any) {
  let status = n;
  if (n === 1) {
    status = 'other';
  } else if (n === 2) {
...
function parseLinuxAudioPciMM(lines: string[], audioPCI: Array<{ slotId?: string; driver?: string }>): Partial<AudioData> {
...
function parseDarwinChannel(str: any) {
  let result = '';
...
function parseDarwinAudio(audioObject: Record<string, string>, id: number): Partial<AudioData> {
  const result: Partial<AudioData> = {};
...
function parseWindowsAudio(lines: string[]): Partial<AudioData> {
  const result: Partial<AudioData> = {};
...
export function audio(callback?: Callback<AudioData[] | null>): Promise<AudioData[] | null> {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        util.powerShell('Get-CimInstance Win32_SoundDevice | select DeviceID,StatusInfo,Name,Manufacturer | fl').then((stdout) => {
          const parts = stdout.toString().split(/\n\s*\n/);
