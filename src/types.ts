...
// types.ts
// ----------------------------------------------------------------------------------
// Description:   System Inspector - Shared type definitions
//                Extracted from index.d.ts for use by all modules
// ----------------------------------------------------------------------------------
// License:       MIT
// ==================================================================================

/**
 * Generic callback type used by all async API functions.
 * Every public function accepts an optional callback as its last argument.
 */
export type Callback<T> = (data: T) => void;

export type MaybeUnsupported<T> = T | null;
...
// 10. "Get All at once" - aggregate types
...
export interface StaticData {
  ...
  cpu: CpuData;
  graphics: MaybeUnsupported<GraphicsData>;
  net: NetworkInterfacesData[];
  memLayout: MemLayoutData[];
  diskLayout: DiskLayoutData[];
  audio: MaybeUnsupported<AudioData[]>;
  bluetooth: MaybeUnsupported<BluetoothDeviceData[]>;
  usb: MaybeUnsupported<UsbData[]>;
  printer: MaybeUnsupported<PrinterData[]>;
}
...
export interface DynamicData {
  ...
  fsStats: FsStatsData;
  disksIO: DisksIoData;
  wifiNetworks: MaybeUnsupported<WifiNetworkData[]>;
  inetLatency: number | null;
}

export type AllData = StaticData & Partial<DynamicData>;
